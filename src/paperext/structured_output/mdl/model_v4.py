...
SYSTEM_MESSAGE = (
    ...
    "unknown (use 'inference' only when the Model is used for inference ONLY, "
    "since training or finetuning always imply later inference), and its "
    "parameter count only when the paper explicitly states it — never infer the "
    "parameter count from the Model name."
)
FIRST_MESSAGE = (
...
class RefModel(BaseModel):
    ...
    parameter_count: Explained[str] = Field(
        description="Number of parameters (model scale, e.g. '7B', '175B') of the "
        "Model, ONLY if explicitly stated in the paper; otherwise 'unknown'. NEVER "
        "infer or guess the parameter count from the Model name or from prior "
        "knowledge; the quote must contain the value stated in the paper.",
    )
    ...
    def __lt__(self, other: "Explained"):
        for (k, v), (ok, ov) in zip(self, other):
            if k != ok:
                break
            if k in ("referenced_paper_title",):
                continue
            if v == ov:
                continue
            return v < ov
...
def empty_model(model_cls):
    ...
    empty_fields["models"][0]["parameter_count"]["value"] = "unknown"
